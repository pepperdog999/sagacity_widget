...
function hide_search_tab(obj,t_index){
	...
	$(".lv-cascade-search li a .lv-narrow-icon").eq(t_index).attr('src', '');
...
function btnClick(){
	$.ajax({
		 type: "POST",
         url:"../phoneSearch/search",
         data:$('#key_search').serialize(),// 要提交的表单
         success: function(data) {
        	var resultList='';
			for(var i=0;i<data.list.length;i++){
				if(i>0){
					resultList+='<div class="lv-seprator-line"></div>';
				}
				resultList+='<li class="lv-spotticket-li clear">';
				resultList+='<div class="lv-spotticket-img lv-pull-left"><img src="img/f_logo.jpg"></div>';
				resultList+='<div class="lv-spotticket-info lv-pull-right"><ul>';
				resultList+='<li class="lv-spotticket-title"><div class="jd_title"><a href="tel:'+data.list[i].MobilePhone1+'">'+data.list[i].Name+'（'+data.list[i].MobilePhone1+'）</a></div></li>';
				resultList+='<li><span class="lv-color">部门</span><i>'+data.list[i].DeptName+'</i><span class="lv-pull-right">'+data.list[i].PosName+'</span></li>';
				resultList+='<li class="lv-ellipsis jd_info">'+data.list[i].CorpName+'</li>';
				resultList+='</ul></div></li>';
			}
			if(data.pageNumber==1){
				$("#data_list").html(resultList);
			}else{
				$("#data_list").append(resultList);
			}
